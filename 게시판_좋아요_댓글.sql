...
insert into board_table (board_title, board_writer, board_contents, member_id, category_id) values ('마흔에 읽는 쇼펜하우어', '강용수', '마흔은 아니지만 쇼펜하아우어어우어ㅝ워', 1,1 );
insert into board_table (board_title, board_writer, board_contents, member_id, category_id) values ('도둑맞은 집중력', '요한 하리', '누가 내 집중력 훔쳐갔어', 1,1 );
insert into board_table (board_title, board_writer, board_contents, member_id, category_id) values ('공지사항', '김삿갓', '벌써 2아웃', 1,2 );

-- 2번 회원이 자유게시판 글 3개 작성
insert into board_table (board_title, board_writer, board_contents, member_id, category_id) values ('자유게시판3', '작성자2', '따라라라랑', 2,1 );
insert into board_table (board_title, board_writer, board_contents, member_id, category_id) values ('자유게시판4', '작성자2', '따란따란', 2,1 );
insert into board_table (board_title, board_writer, board_contents, member_id, category_id) values ('자유게시판5', '작성자2', '빠밤빠밤', 2,1 );

-- 3번 회원이 가입인사 글 1개 작성 
insert into board_table (board_title, board_writer, board_contents, member_id, category_id) values ('가입인사1', '작성자3', '가입환영', 4,3 );

-- 1.1. 게시글 작성(파일첨부 o)
-- 2번 회원이 파일있는 자유게시판 글 2개 작성
insert into board_table (board_title, board_writer, board_contents, board_file_attached, member_id, category_id) values ('자유게시판6', '작성자2', '따라라라랑',1, 2,1 );
insert into board_table (board_title, board_writer, board_contents, member_id, board_file_attached, category_id) values ('자유게시판7', '작성자2', '따란따란', 1,2,1 );
insert into board_table (board_title, board_writer, board_contents, member_id, board_file_attached, category_id)  (select board_title, board_writer, board_contents, member_id, board_file_attached, category_id from board_table);
...
select b.*, c.category_name from board_table b, category_table c ;
...
insert into board_file_table (orginal_file_name, stored_file_name, board_id) values ('사진1','사진1.jpg',1);

select * from board_table b , borad_file_table f where b.board_file_attached = 1;
-- 4. 1번 회원이 자유게시판에 첫번째로 작성한 게시글의 제목, 내용 수정
-- 5. 2번 회원이 자유게시판에 첫번째로 작성한 게시글 삭제 
-- 7. 페이징 처리(한 페이지당 글 3개씩)
select * from board_table order by id desc;
select * from board_table order by id desc limit 0, 3; -- 35 36 37 0번째부터 3개 1page
select * from board_table order by id desc limit 3, 3; -- 32 33 34 3번 부터 3개 2page
select * from board_table order by id desc limit 6, 3; -- 29 30 31 6번부터 3개  3page
select * from board_table order by id desc limit 12, 3; -- 
select * from board_table order by id desc limit 0, 5;
-- 한 페이지당 3개씩 출력하는 경우 전체 글 갯수가 20개 라면 필요한 페이지 갯수는 ? 7개
...
insert into comment_table (comment_writer, comment_contents, board_id, member_id) values ('댓','댓글',16,1);
-- 1.2. 2번 회원이 1번 게시글에 댓글 작성 
insert into comment_table (comment_writer, comment_contents, board_id, member_id) values ('댓','댓글',17,2);
-- 2. 댓글 조회
select * from comment_table;
select * from board_table where id =1;
select * from board_table b, comment_table c  where b.id =c.board_id and b.id=16;
...
insert into good_table (comment_id, member_id) values (6,1);

-- 1번 회원이 2번 댓글에 좋아요를 했는지 체크
...
insert into good_table (comment_id, member_id) values (6,4);
-- 4. 댓글 조회시 좋아요 갯수도 함께 조회
select c.*, count(*) from comment_table c, good_table g where c.id = g.comment_id group by c.id;
